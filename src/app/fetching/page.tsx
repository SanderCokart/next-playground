...
export default async function FetchingPage() {
  ...
  return (
    <main>
      <div className="grid grid-cols-[repeat(auto-fill,minmax(300px,1fr))] gap-8 p-8">
        {topics.map(({ topic, description, workInProgress, href }) => (
          <Link key={topic} className={cn(workInProgress && 'cursor-not-allowed')} href={workInProgress ? '' : href}>
